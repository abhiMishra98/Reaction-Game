...
typedef enum {
    BTN_UNDEF,       //!< Undefined state
    BTN_LOW,         //!< Low state
    BTN_RISING,      //!< Rising Edge state
    BTN_HIGH,        //!< High state
    BTN_FALLING      //!< Falling Edge state
} BTN_state_t;

/**
* \brief Type definition for gpio read function
*/
typedef uint8_t (*BTN_gpio_read_t)();
...
typedef struct {
    BTN_state_t      m_prevState;    //!< the previous switch state, required for detecting edges
    BTN_gpio_read_t  m_read;         //!< Function pointer for the switch read function 
}BTN_t;
